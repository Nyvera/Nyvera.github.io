...
async function handleInput() {
  ...
  if (!navigator.onLine) {
    addMessage('ai', offlineReplies[Math.floor(Math.random() * offlineReplies.length)]);
    return;
  }
  ...
  if (/speak|say/i.test(prompt)) return generateSpeech(prompt.replace(/^(speak|say)\s*/i, ''));
...
function handleFileUpload(event) {
  ...
  if (fileType.startsWith('image/')) {
    ...
    const url = URL.createObjectURL(file);
    addMessage('user', `Uploaded document: ${fileName}`);
    addMessage('ai', `<iframe src="${url}" style="width: 100%; height: 400px; border: 1px solid #ddd; border-radius: 8px;"></iframe>`);
  } else {
